...
export const FullStackDockerDeployment = ({ data }: { data?: any }) => {
  ...
  const dockerProfile = constructTemplates ? 'all' : 'no_jd';
  const commandSequence = `docker compose --profile ${dockerProfile} up --build`;
...
cd docker`}
      ...
      <div className="grid gap-4 md:grid-cols-1">
        <InfoCard number={4} title="Download .env file" icon={FileDown}>
          <p className="text-sm text-muted-foreground mb-3">
            Download the generated <code className="text-xs">.env</code> file and place it in the <code className="text-xs">sv2-apps/docker</code> directory.
          </p>
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 mb-3">
            <p className="text-xs text-amber-200 mb-2">
              <strong>Important:</strong> The file may download as <code className="text-xs">env</code> (without the dot). After downloading, rename it to <code className="text-xs">.env</code> before placing it in the <code className="text-xs">sv2-apps/docker</code> directory.
            </p>
            <p className="text-xs text-amber-200 font-mono bg-black/20 p-2 rounded mt-2">
              mv ~/Downloads/env ~/Downloads/.env
            </p>
            <p className="text-xs text-amber-200/80 mt-1">
              Or rename it manually in Finder/File Explorer.
            </p>
          </div>
          <Button 
            variant="secondary" 
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
              <span className="text-sm text-white">
                I have downloaded, renamed to <code className="text-primary font-semibold">.env</code> (if needed), and placed the file in <code className="text-primary font-semibold">sv2-apps/docker/</code>
              </span>
          </label>
        </div>
      </div>
