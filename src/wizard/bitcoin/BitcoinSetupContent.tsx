...
export const BitcoinSetupContent = ({ 
  ...
  // Start with empty value so placeholder is visible, use saved value or empty string
  ...
  const startNodeCommand = network === "mainnet" 
    ...
    : network === "testnet4"
    ? "bitcoin -m node -ipcbind=unix -testnet4"
    : "bitcoin -m node -ipcbind=unix -signet";

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (updateData) {
      updateData({ bitcoinSocketPath: socketPath });
    }
    if (onContinue) {
      onContinue();
    }
            ...
            <div>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <code className="block bg-black/30 px-3 py-2 rounded text-sm font-mono text-primary/90">~/.bitcoin/bitcoin.conf</code>
                </div>
                <div>
                  <span className="text-xs text-muted-foreground block mb-1">macOS</span>
                  <code className="block bg-black/30 px-3 py-2 rounded text-sm font-mono text-primary/90">~/Library/Application Support/Bitcoin/bitcoin.conf</code>
                </div>
              </div>
            </div>
            <div>
      ...
      {!nodeStarted ? (
        ...
        <div className="border border-primary/20 rounded-lg p-6 bg-primary/5">
          ...
          <div className="space-y-3 mb-6">
            <div>
              <span className="text-xs uppercase tracking-widest text-muted-foreground block mb-1">Linux / Unix</span>
              <code className="block bg-black/30 px-3 py-2 rounded text-sm font-mono text-primary/90 break-all">{networkSocket.path}</code>
            </div>
            <div>
              <span className="text-xs uppercase tracking-widest text-muted-foreground block mb-1">macOS</span>
              <code className="block bg-black/30 px-3 py-2 rounded text-sm font-mono text-primary/90 break-all">{networkSocket.macPath}</code>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="socketPath" className="text-white font-medium">
                Bitcoin Socket Path <span className="text-primary">*</span>
              </Label>
              <Input 
                id="socketPath" 
                placeholder={networkSocket.path}
                value={socketPath} 
              ...
              />
              <p className="text-xs text-muted-foreground">
                <strong className="text-white/80">Enter or modify</strong> the full path to your <code className="text-primary">node.sock</code> file. This will be used later for deployment configuration.
              </p>
            </div>
